...
<body>
    ...
    <main class="container">
        ...
        <div>
            <div>
                <form action="{{route('Permisos.update_user',$User->id)}}" method="POST" class="d-flex gap-5 mt-5">
                    @csrf
                    @method('PUT')
                    <div class="row ancho">
                        <div class="col d-flex flex-column gap-4">
                            @foreach ($TiposPermisos as $item)
                            <div class="">
                                <input name="permiso[]" value="{{ $item->id }}" type="checkbox">
        ...
        <div class="mt-5">
            <table>
                ...
                <tbody>
                    ...
                    <td>{{ $User->email }}</td>
                    @if($User->activo == 1)
                    <td class="usuario-activo">Activo</td>
                    @else
                    <td class="usuario-activo">Inactivo</td>
                    @endif
                </tbody>
            </table>
        </div>
